#!/usr/bin/env python3
"""
WADI BulletProofs 빠른 프로덕션 테스트
각 조건당 100개씩으로 단축 테스트
"""
...
class QuickProductionTest:
    ...
    def __init__(self):
        ...
        print(f"⚡ 빠른 프로덕션 테스트 준비")
        print(f"🎯 16조건 × 100요청 = 1,600개 요청")

    def run_condition(self, condition_id: int, sensor_count: int, frequency: int, num_requests: int = 100) -> ExperimentResult:
        """단일 조건 실행"""
        print(f"조건 {condition_id:2d}: {sensor_count:3d}센서×{frequency:3d}Hz×{num_requests}요청", end=" → ")
        ...
        print(f"성공률: {result.success_rate:5.0f}%, 시간: {result.avg_total_time:6.1f}ms, 소요: {duration:5.1f}초")
        return result

    def run_quick_experiment(self):
        """빠른 실험 실행"""
        print("⚡ === WADI BulletProofs 빠른 프로덕션 테스트 ===")
        ...
        for sensor_count in sensor_counts:
            ...
            for frequency in frequencies:
                try:
                    result = self.run_condition(condition_id, sensor_count, frequency, 100)
        ...
        csv_path = f"wadi_bulletproofs_quick_{timestamp}.csv"
        ...
        if overall_success_rate >= 95:
            print(f"\n🎉 빠른 테스트 성공! 1000개 요청 실험 준비 완료")
            print(f"⏰ 예상 1000개 실험 시간: {total_duration*10/60:.0f}분")
        else:
            print(f"\n⚠️ 빠른 테스트에서 문제 발견. 성공률: {overall_success_rate:.1f}%")

        return csv_path

def main():
    test = QuickProductionTest()
    test.run_quick_experiment()
